...
const SignalPostPage = ({ signal: slug }: Props) => {
    ...
    return (
      <main className="min-h-screen bg-black text-zinc-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-medium text-white mb-4">Signal not found</h1>
  ...
  const prevSignal = currentIndex < signals.length - 1 ? signals[currentIndex + 1] : null;
      ...
      <article className="pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-3xl mx-auto">
          ...
          ease: [0.16, 1, 0.3, 1]
            ...
            {signal.body.split('\n\n').map((paragraph, index) => <p key={index} className="text-lg md:text-xl text-zinc-300 leading-relaxed mb-8">
                {paragraph}
              </p>)}
          </motion.div>
          ...
          delay: 0.4
            ...
            <div className="flex flex-row justify-between gap-6">
              <Link href="/signals" className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-300 transition-colors group">
                <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
                All signals
              </Link>

              <div className="flex flex-col sm:flex-row gap-6">
                {prevSignal && <Link href={`/signals/${prevSignal.slug}`} className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-300 transition-colors group">
                    <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
                    Previous
                  </Link>}
                {nextSignal && <Link href={`/signals/${nextSignal.slug}`} className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-300 transition-colors group">
                    Next
